...
import { Candidate } from '@/lib/data'; // تم حذف getAllVotes لأنه لم يعد موجوداً
...
export function Top5Section({ title, genderLabel, candidates, lang, votesLabel, onSelect }: Top5Props) {
  ...
  // ملاحظة: تم حذف سطر const votes = getAllVotes(); 
  // لأن الأصوات الآن تأتي محقونة مباشرة داخل كائن الـ candidate من خلال ملف data.ts المحدث
          ...
          >
            ...
            <div className="p-3 text-start">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {/* عرض الأصوات المحدثة من السيرفر */}
                {c.votes ?? 0} {votesLabel}
              </p>
            </div>
          </button>
        ))}
