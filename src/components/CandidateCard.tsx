...
export function CandidateCard({ candidate, lang, rank, votesLabel, votedLabel, onSelect }: CandidateCardProps) {
  // 1. جعل عدد الأصوات يعتمد على حالة (State) ليتحدث فور جلب البيانات
  const [votes, setVotes] = useState(candidate.votes || 0);
  
  // 2. التحقق مما إذا كان المستخدم قد صوت لهذا المتسابق تحديداً
  const votedForThis = getVotedCandidateId(candidate.gender) === candidate.id;
  const name = candidate.name;

  // 3. مزامنة عدد الأصوات عند تغيير البيانات القادمة من السيرفر (refreshKey)
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        {/* مؤشر التصويت (يظهر إذا كان المستخدم قد اختار هذا المتسابق) */}
        {votedForThis && (
          <div className="absolute top-3 end-3 flex h-9 w-9 items-center justify-center rounded-xl bg-gold/90 text-primary-foreground shadow-lg animate-in zoom-in duration-300">
            <Check className="h-5 w-5" />
          </div>
        )}

        {/* غطاء عند التمرير - View Profile */}
      ...
      <div className="p-4 text-start">
        ...
        <div className="mt-1 flex items-center justify-between">
          {/* عرض عدد الأصوات المحدث من السيرفر */}
