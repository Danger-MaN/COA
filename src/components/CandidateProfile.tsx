...
import { candidates, getVotes, fetchLiveVotes, Candidate } from '@/lib/data';
...
const CandidatePage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // جلب جميع المرشحين من نفس الجنس مع الأصوات الحية
      const sameGenderCandidates = candidates.filter(c => c.gender === base.gender);
      const withLiveVotes = await Promise.all(
        sameGenderCandidates.map(async (c) => {
      ...
      // الترتيب تنازلياً حسب الأصوات
      const sorted = withLiveVotes.sort((a, b) => (b.votes || 0) - (a.votes || 0));
      const found = sorted.find(c => c.id === id);
      if (found) {
        setCandidateWithVotes(found);
        setRank(sorted.findIndex(c => c.id === id));
      } else {
        // إذا لم يوجد (نادر) نعرض البيانات الأساسية
        setCandidateWithVotes({ ...base, votes: getVotes(base.id) });
        setRank(-1);
      }
    } catch (err) {
      console.error(err);
